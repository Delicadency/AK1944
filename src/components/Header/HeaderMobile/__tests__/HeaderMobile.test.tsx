...
describe("HeaderMobile", () => {
  ...
  it("renders the text with correct classes", () => {
    render(<HeaderMobile/>);
    expect(screen.getByText("Światowy Związek Żołnierzy AK")).toHaveClass("h-8 w-[135px] text-center font-courier text-14 text-white contrast:text-black00")
  })
  ...
  it("renders the menu with correct classes", () =>{
    render(<HeaderMobile/>);
    expect(screen.getByTestId("mobile-menu")).toHaveClass("fixed right-0 top-0 z-50 h-full w-full transform transition-transform duration-300 ease-in-out bg-greenB")
  });
